...
def relocate_elf_library(patchelf, output_dir, output_library, binary):
    ...
    new_libraries_path = osp.join(output_dir, "torchaudio.libs")
    ...
    subprocess.check_output(
        [patchelf, "--set-rpath", "$ORIGIN:$ORIGIN/../../torchaudio.libs", binary_path], cwd=output_library
    )
...
def relocate_dll_library(dumpbin, output_dir, output_library, binary):
    ...
    print("Copying dependencies to wheel directory")
    package_dir = osp.join(output_dir, "torchaudio")
    for library in binary_paths:
        if library != binary:
            library_path = binary_paths[library]
...
def patch_linux():
    ...
    libtorchaudio_binary = "libtorchaudio.so"
    torchaudio_binaries = [libtorchaudio_binary]
    for wheel in wheels:
        ...
        output_library = osp.join(output_dir, "torchaudio", "lib")
        for binary in torchaudio_binaries:
...
def patch_win():
    ...
    libtorchaudio_binary = "libtorchaudio.pyd"
    torchaudio_binaries = [libtorchaudio_binary]
    for wheel in wheels:
        if osp.exists(output_dir):
            shutil.rmtree(output_dir)
        ...
        for binary in torchaudio_binaries:
            if osp.exists(osp.join(output_library, binary)):
                relocate_dll_library(dumpbin, output_library, binary)
